...
#define STDIN  (eDEV_KBD)

#define READER_BUFFER_SIZE (256)

typedef struct _sram_dev{
    uint16_t first; //position début fichier
    uint16_t pos; // position courante
    uint16_t fsize; // grandeur fichier
}sram_device_t;


typedef struct _flash_dev{
    long unsigned int saddr; //position début dans mémoire flash
    long unsigned int pos; // position courante dans mémoire flash
    long unsigned fsize; // grandeur fichier
}flash_device_t;
