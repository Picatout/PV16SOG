...
static void display_prompt(){
    int i;
    put_char('\n');
    for(i=0;i<=complevel;i++) put_char('>');
}//f
...
static bool fill_buffer(reader_t *reader){
    switch (reader->device){
        case STDIN:
            display_prompt();
            reader->count=readln(reader->buffer,CHAR_PER_LINE);
        ...
        case eDEV_SPIRAM:
            ...
            break;
        case eDEV_SDCARD:
            reader->count=fat_read_file(reader->handle,(uint8_t*)reader->buffer,READER_BUFFER_SIZE);
            reader->inp=0;
            if (reader->count<=0){
                reader->count=0;
                reader->eof=true;
